...
void Background::updatePosition() {
    ...
    third_coordinate.changeInXCoordinate(-Config::config()->getStickman()->getMovementVelocity());
}

void Background::restoreState(std::shared_ptr<Background> backgroundMemento) {
    this->main_coordinate.setXCoordinateToZero(backgroundMemento->main_coordinate.getXCoordinate());
    this->main_coordinate.setYCoordinateToZero(backgroundMemento->main_coordinate.getYCoordinate());

    this->first_coordinate.setXCoordinateToZero(backgroundMemento->first_coordinate.getXCoordinate());
    this->first_coordinate.setYCoordinateToZero(backgroundMemento->first_coordinate.getYCoordinate());

    this->second_coordinate.setXCoordinateToZero(backgroundMemento->second_coordinate.getXCoordinate());
    this->second_coordinate.setYCoordinateToZero(backgroundMemento->second_coordinate.getYCoordinate());

    this->third_coordinate.setXCoordinateToZero(backgroundMemento->third_coordinate.getXCoordinate());
    this->third_coordinate.setYCoordinateToZero(backgroundMemento->third_coordinate.getYCoordinate());

    this->backgroundVelocity = backgroundMemento->backgroundVelocity;
    this->first = backgroundMemento->first;
    this->second = backgroundMemento->second;
    this->third = backgroundMemento->third;
}
